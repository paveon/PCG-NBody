...
int main(int argc, char **argv) {
    ...
    size_t sharedMemorySize = t_particles_member_count * sizeof(float) * blockSize.x;
    
    for (size_t s = 0; s < steps; s++) {
        calculate_velocity<<<gridSize, blockSize, sharedMemorySize>>>(particles_gpu[s & 1ul],
                                                                      particles_gpu[(s + 1) & 1ul], N, dt);
